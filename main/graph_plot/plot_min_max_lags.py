...
def main():
    ...
    lags = {
        'engelsa_37.csv.pickle': [4, 6, 5],
        # 'gaydara_30.csv.pickle': [2, 5, 3],
        # 'gaydara_32.csv.pickle': [3, 5, 3]
    }
    home_column = "HOME"
    ...
    for home_dataset_name in os.listdir(config.HOMES_PREPROCESSED_HEATING_CIRCUIT_DATASETS_DIR):
        if home_dataset_name in lags.keys():
            ...
            for i in range(3):
                ...
                home_lag = lags.get(home_dataset_name)[i]
                ...
                # id_date_boiler = boiler_df[column_names.TIMESTAMP]
                # id_date_home = id_date_boiler + lags.get(home_dataset_name)[i]
                # id_temp_home = home_forward_temp[id_date_home]
